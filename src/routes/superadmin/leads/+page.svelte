        ...
        <tbody>
          {#each leads as lead}
            <tr class="border-b border-border last:border-0 hover:bg-muted/10">
              ...
              <td class="p-3">{lead.platformFee != null ? formatSYP(lead.platformFee) : "—"}</td>
              <td class="p-3">
                <div class="flex gap-1">
                  ...
                  {/if}
                  <Button size="sm" variant="ghost" onclick={() => del(lead.id)} class="h-7 px-2 text-destructive hover:bg-destructive/10">
                    <Trash2 size={14} />
                  </Button>
                </div>
              </td>
